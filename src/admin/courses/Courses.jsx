...
export default function Courses() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const arr = Array.isArray(data) ? data.map(normalizeCourse).filter(Boolean) : [];
  ...
  const filtered = useMemo(() => {
    ...
    if (addressFilter !== 'all') arr = arr.filter((c) => c.address === addressFilter);

    arr.sort((a, b) => {
      let res = 0;
      if (sortKey === 'price') res = (a.price || 0) - (b.price || 0);
      if (sortKey === 'createdAt') res = new Date(a.createdAt || 0) - new Date(b.createdAt || 0);
  ...
  const bulkDelete = async () => {
    ...
    if (!confirm(`Delete ${selected.length} selected course(s)?`)) return;
    const token = localStorage.getItem('token');
    try {
      // run delete requests in parallel
      const promises = selected.map((id) => apiDeleteCourse(id, token).catch((e) => ({ error: e, id })));
      ...
      for (const r of results) {
        if (r && r.error) {
          ...
          if (r.ok) {
            // we don't have id here; rely on selected
            // handled below by removing all selected except failed
          } else {
            // you could parse r to get error
            // difficult to map; mark as failed
            // add to failed list (best-effort)
      ...
      setSelected([]);
      if (failed.length > 0) {
        alert(`Failed to delete ${failed.length} course(s). See console for details.`);
  ...
  const toggleActive = async (id) => {
    ...
    try {
      ...
      setCourses((prev) => prev.map((c) => (c.id === id ? { ...c, ...norm } : c)));
    } catch (err) {
      console.error('Failed to toggle active', err);
      alert('Failed to toggle active. See console for details.');
    }
  };
  ...
  const bulkToggleActive = async () => {
    ...
    try {
      const promises = selected.map((id) =>
        apiPatchCourse(id, { active: true }, token).catch((e) => ({ error: e, id }))
      );
      const results = await Promise.all(promises);
      ...
      if (updatedIds.length === 0 && failed.length === 0) {
        ...
        setCourses((prev) => prev.map((c) => (selected.includes(c.id) ? { ...c, active: true } : c)));
      } else {
        setCourses((prev) => prev.map((c) => (updatedIds.includes(String(c.id)) ? { ...c, active: true } : c)));
      }
      setSelected([]);
      if (failed.length > 0) {
        alert(`Failed to mark ${failed.length} course(s) active. See console.`);
        console.error('Failed ids: ', failed);
      }
    } catch (err) {
      console.error('bulkToggleActive failed', err);
      alert('Failed to mark selected active. See console for details.');
    }
  };
  ...
  const duplicateCourse = async (id) => {
    ...
    try {
      ...
      alert('Failed to duplicate. See console for details.');
    }
  };
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="mb-6 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <div className="text-sm text-gray-600">
                  <div>Total: <span className="font-medium text-gray-900">{courses.length}</span></div>
                </div>
                <Link to="/admin/courses/create" className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">
            ...
            <div className="mb-4 flex flex-col md:flex-row gap-3 items-stretch">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder="Izlash... (Uz / Ru / address)"
                  value={search}
              ...
              <select value={addressFilter} onChange={(e) => setAddressFilter(e.target.value)} className="px-3 py-2 border rounded-lg">
                {addresses.map((a) => (
                  <option key={a} value={a}>{a === 'all' ? 'Barcha manzillar' : a}</option>
                ))}
              </select>
              ...
              <div className="flex items-center gap-2">
                ...
                <select value={sortKey} onChange={(e) => setSortKey(e.target.value)} className="px-2 py-2 border rounded-lg">
                  ...
                  <option value="uz">Name (Uz)</option>
                </select>

                <button className="px-2 py-2 border rounded-lg" onClick={() => setSortDir((d) => (d === 'asc' ? 'desc' : 'asc'))}>
                  {sortDir === 'asc' ? '↑' : '↓'}
                </button>
              </div>
            </div>
            ...
            <div className="mb-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <button onClick={bulkToggleActive} className="px-2 py-1 text-sm border rounded-md">Mark active</button>
              ...
              {filtered.map((c) => (
                <div key={c.id} className="bg-white border rounded-xl p-4 shadow-sm hover:shadow-md transition">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <div className="flex gap-2 items-center">
                          <h3 className="text-lg font-semibold text-gray-900">Uz: {c.uz}</h3>
                          {c.active && <span className="px-2 py-1 text-xs bg-green-100 text-green-700 rounded-full">Faol</span>}
                        </div>
                  ...
                  <div className="mt-4 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <button onClick={() => navigate(`/admin/course/create?id=${c.id}`)} title="Edit" className="px-2 py-1 border rounded-md text-sm flex items-center gap-2">
                        <Edit size={14}/> Edit
                      </button>
                      <button onClick={() => duplicateCourse(c.id)} className="px-2 py-1 border rounded-md text-sm">Duplicate</button>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => deleteCourse(c.id)} className="px-2 py-1 border rounded-md text-sm text-red-600">
                        <Trash2 size={14}/>
                      </button>
                    </div>
                  </div>
            ...
            {/* empty */}
            {filtered.length === 0 && (
              <div className="mt-6 text-center text-gray-500">No courses found. Try clearing filters or add a new course.</div>
            )}
          </div>
        </main>
      </div>
